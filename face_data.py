...
if __name__=='__main__':
    load_model = False
    save_model_prefix = 'paramters/face_matchnet_elemental_pro/face_matchnet'
    #read data from caffe
    batch_size = 128
    #load the data and initialize the dataIter
    test_data = fds.DateIter(['left_data', 'right_data'],
                             [(batch_size, 1, 64, 64), (batch_size, 1, 64, 64)], 'test',
                             ['label'], [(batch_size,)])
    data = fds.DateIter(['left_data', 'right_data'],
                  [(batch_size,1,64,64), (batch_size,1,64,64)],'train',
                  ['label'], [(batch_size,)])
    ...
    if load_model:
        ...
        network = matn.get_match_innerproduct_net()
        model = mx.model.FeedForward(
            ctx=devs,
            symbol=network,
            num_epoch=1000,
